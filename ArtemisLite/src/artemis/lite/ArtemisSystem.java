...
/**
 * This represents a System of Components within the Game.
 *
...
public class ArtemisSystem {
	...
	/**
	 * Loops through the components within the system and determines how many more are required before the player can
	 * begin developing each component.
	 *
	 * @param player the player to check component ownership against
	 */
	public void announceComponentsRequiredBeforeDevelopment(Player player) throws IllegalArgumentException {
		if (player == null) {
			throw new IllegalArgumentException("Player is null");
		}

		int totalComponents = this.getComponentsInSystem().size();
		int totalOwnedByPlayer = 0;

		for (Component component : componentsInSystem) {
			if (component.getComponentOwner() == player) {
				totalOwnedByPlayer++;
			}
		}

		int remainingComponentsRequired = totalComponents - totalOwnedByPlayer;

		if (remainingComponentsRequired != 0) {
			Game.announce(String.format("%s you need %s more components before you can begin developing each component within the %s",
					player.getPlayerName().toUpperCase(), remainingComponentsRequired, systemName));
		}
	}
